...
const App = () => {
  // STATE: Store current user object. Initializes by checking localStorage for an active session.
  const [currentUser, setCurrentUser] = useState(() => {
    const savedSession = localStorage.getItem('ecosta_active_session');
    return savedSession ? JSON.parse(savedSession) : null;
  });
  
  // STATE: Manage map selection, routing, and full-screen image viewing
  const [selectedSection, setSelectedSection] = useState(null);
  const [activeRoute, setActiveRoute] = useState(null);
  const [modalImage, setModalImage] = useState(null);
  
  // STATE: Manage global stadium selection
  ...
  const handleSectionClick = (id) => {
    if (selectedSection === id) {
      ...
      // Select new section and clear any active route
  ...
  const handleLogout = () => {
    ...
    setCurrentUser(null); 
    // Clear active session from browser memory
  ...
  // RENDER: Show Landing/Auth page if no user is currently logged in
  if (!currentUser) {
    return <AuthFlow onLoginSuccess={handleLoginSuccess} />;
  }

  // RENDER: Main Application (Protected Route)
  return (
    <div className="h-screen w-full flex flex-col font-sans bg-slate-950 text-slate-200 overflow-hidden relative animate-app-enter">
      ...
      {/* HEADER: High z-index (40) ensures the account dropdown is not clipped by the ViewPanel */}
      <header className="flex flex-shrink-0 items-center justify-between px-6 py-4 bg-slate-900 border-b border-slate-800 shadow-md z-40">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            {/* Avatar Button: Displays the first letter of the user's name */}
            <button 
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        {/* Right Side: Information & Views Panel */}
      ...
      <style>{`
        ...
        /* CRITICAL FIX FOR VERCEL MAP BUG: 
          Removed 'transform: scale()' from this animation. 
          The container must maintain 100% dimensions from second 0 so the map library 
          calculates the exact center coordinate properly.
        */
        @keyframes app-enter { 
